...
class UserServiceImplTest {
    ...
    void testAdminLogin(){
        ...
        when(userRepository.findByUsernameAndPasswordAndRole("admin","admin", User.Role.ADMIN))
                .thenReturn(Optional.of(user));

        Optional<UserDto> actual = underTest.adminLogin("admin","admin");

        assertEquals(expected, actual);
        verify(userRepository).findByUsernameAndPasswordAndRole("admin","admin", User.Role.ADMIN);
